...
class PullRequestHandler:
    ...
    def __call__(self, payload: dict) -> HttpResponse:
        ...
        Returns:
            An object containing the primary keys of the pull request and bill affected, if applicable
        """
        handler: Callable[[dict[str, Any]], tuple[PullRequest | None, Bill | None]] | None
    ...
    def get_response(self, action: str, pull_request: PullRequest | None, bill: Bill | None) -> JsonResponse:
...
class GithubWebhookView(View):
    ...
    def _validate_header(headers: request.HttpHeaders) -> HttpResponseBadRequest | None:
        ...
        if header_signature is None:
            return HttpResponseBadRequest("Request does not contain X-HUB-SIGNATURE-256 header")
    ...
    @staticmethod
    def _validate_signature(header_signature: str, request_body: bytes) -> HttpResponse | None:
        ...
        if digest_name != "sha256":
            return HttpResponseBadRequest(f"Unsupported X-HUB-SIGNATURE-256 digest mode: {digest_name}")

        mac = hmac.new(settings.WEBISCITE_GITHUB_WEBHOOK_SECRET.encode("utf-8"), msg=request_body, digestmod="sha256")
    ...
    def post(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        event = request.headers.get("x-github-event", "ping")  # header was already validated
        handler = getattr(self, event, None)
        if handler is None:
            return HttpResponseBadRequest(f"Unsupported X-GITHUB-EVENT header found: {event}")

        payload = json.loads(request.body.decode("utf-8"))
        return handler(payload)

    def ping(self, payload: dict) -> HttpResponse:  # pylint: disable=unused-argument
        return HttpResponse("pong")

    def push(self, payload: dict) -> HttpResponse:  # pylint: disable=unused-argument
