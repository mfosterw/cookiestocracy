...
from democrasite.social.tests.factories import NoteFactory
...
class TestBaseTemplate:
    def test_base_no_person(self, user: User, rf: RequestFactory):
        ...
        response = render(request, "social/base.html")
...
class TestNoteListTemplate:
    def test_no_notes(self, client: Client):
        response = client.get(reverse("social:note-list"))

        assert response.status_code == HTTPStatus.OK
        assert response.templates[0].name == "social/note_list.html"
    ...
    def test_with_notes(self, note: Note, client: Client):
        ...
        content = client.get(reverse("social:note-list")).content.decode()
    ...
    def test_reply_link(self, note: Note, user: User, person: Person, client: Client):
        ...
        assert reverse("social:note-reply", kwargs={"pk": note.id}) not in content
        ...
        assert reverse("social:note-reply", kwargs={"pk": note.id}) in content
...
class TestNoteDetailTemplate:
    def test_note_detail(self, note: Note, client: Client):
        response = client.get(reverse("social:note-detail", kwargs={"pk": note.id}))

        assert response.status_code == HTTPStatus.OK
        assert response.templates[0].name == "social/note_detail.html"
    ...
    def test_note_ancestors(self, note: Note, client: Client):
        ...
        content = client.get(
            reverse("social:note-detail", kwargs={"pk": reply2.id})
        ).content.decode()
...
class TestNoteFormTemplate:
    def test_note_form(self, person: Person, client: Client):
        ...
        response = client.get(reverse("social:note-create"))

        assert response.status_code == HTTPStatus.OK
        assert response.templates[0].name == "social/note_form.html"
        assert b"Content" in response.content

    def test_note_reply_form(self, note: Note, person: Person, client: Client):
        client.force_login(person.user)

        response = client.get(reverse("social:note-reply", kwargs={"pk": note.id}))
...
class TestPersonDetailTemplate:
    def test_person_detail(self, person: Person, client: Client):
        ...
        assert response.status_code == HTTPStatus.OK
        assert response.templates[0].name == "social/person_detail.html"
    ...
    def test_edit_link(self, person: Person, client: Client):
        ...
        assert response.status_code == HTTPStatus.OK
        assert reverse("social:person-update") in response.content.decode()
...
class TestPersonFormTemplate:
    def test_person_update_form(self, person: Person, client: Client):
        ...
        response = client.get(reverse("social:person-update"))

        assert response.status_code == HTTPStatus.OK
        assert response.templates[0].name == "social/person_form.html"
