...
from democrasite.social import views
...
class TestUserProfileMixin:
    ...
    def test_no_auth(self, rf: RequestFactory):
        ...
        assert response.url == reverse("social:note-list")
...
class TestPersonCreateView:
    ...
    def test_unauthenticated(self, rf: RequestFactory):
        ...
        assert (
            response.url
            == f"{reverse(settings.LOGIN_URL)}?next={reverse('social:note-list')}"
        )
    ...
    def test_person_exists(self, rf: RequestFactory, person: Person):
        ...
        assert (
            response.url
            == f"{reverse('social:person-detail', args=[person.display_name])}"
        )
    ...
    def test_create_person(self, user: User, client: Client):
        ...
        response = client.post(reverse("social:person-create"))
